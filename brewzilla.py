...
class Monitor(tk.Frame):
	def __init__(self, parent, controller):
		...
		self.start_timer = self.pause_timer =self.bzr = self.step_count = 0
		self.set_temp = 71
		...
		self.auto_flag = 1
		self.temp = 0
		self.boil_threshold_temp = 98
		self.cal_temp_toggle = False
		...
		self.timer_label_text.set("1:00:00")
		self.temp_label_text.set("00c")
		...
		calibration_btn = ttk.Button(self, text="Calibrate Temp" , command=lambda: self.cal_temp())
		calibration_btn.place(x=650, y=400)
	...
	def cal_temp(self):
		self.cal_temp_toggle = not self.cal_temp_toggle
		if(self.cal_temp_toggle):
			self.atemp_label.place(x=100,y=190)
		else:
			self.atemp_label.place_forget()
	...
	def read_data(self):
		...
		if(self.button_active() and not self.auto_flag):
	...
	def button_active(self):
		if (time.time() >= self.btn_press_timer + 0.3):
			self.btn_press_count = 1
		if(time.time() >= self.btn_press_timer + 2):
			return False
		else:
			return True
	...
	def get_boil_additions(self):
		...
		while 1:
			try:
				...
				c = round(self.recipe['<HOP>'+str(self.step_count),'<TIME>'])
				if(c < (self.recipe['<RECIPE>0', '<BOIL_TIME>'] * 60) + 60):
					c = 'g'+u'\u2000'+str(c)+'min'
					x =''
					rng = 20 - a_size
					for i in range(rng):
		...
		while 1:
			try:
				...
				c = 'g'+u'\u2000'+str(self.recipe['<MISC>'+str(self.step_count),'<TIME>'])+'min'
				x =''
				rng = 20 - a_size
				for i in range(rng):
					x +=u'\u2000'
		...
		if(not self.auto_flag):
			...
			if (self.complete_flag):
				...
				if (self.toggle):
					...
					if self.pause_timer > 0:
						...
						self.pause_timer = 0
						self.notes_msg_text.set('Timer has restarted...')
				else:
					self.complete_btn.config(text="Play")
			...
			if self.toggle:
				if self.wait_for_temp(self.set_temp):
					if (not self.lock):
						self.notes_msg_text.set('Temp reached '+str(self.set_temp)+' timer has started...')
	...
	def state_machine(self):
		...
		# boil
			...
			self.notes_label_text.set('Boil for '+str(self.recipe['<RECIPE>0', '<BOIL_TIME>'])+'min')
			if(self.wait_for_temp(self.boil_threshold_temp)):
